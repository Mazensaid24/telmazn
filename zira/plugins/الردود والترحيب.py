...
ZelzalWF_cmd = (
    "𓆩 [𝙎𝙊𝙐𝙍𝘾𝙀 𝗞𝗮𝗧𝘆𝗔 - اوامـر الـردود / والتـرحيب](t.me/Source_Katya) 𓆪\n\n"
    ...
    "\n 𓆩 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗞𝗮𝗧𝘆𝗔](t.me/Source_Katya) 𓆪"
)
...
async def on_snip_list(event):
    ...
    for filt in filters:
        if OUT_STR == "** ❈╎لاتوجـد ردود محفوظـه في هـذه الدردشـه ༗**":
            OUT_STR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝙀 𝗞𝗮𝗧𝘆𝗔- قائمـة الـردود 𓆪\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
        OUT_STR += "👉 `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
...
async def _(event):
    ...
    if gvarstatus("TIME_STOP") is not None: #Code by T.me/A7_M3
