...
ZelzalDV_cmd = (
    "[ᯓ 𝙎𝙊𝙐𝙍𝘾𝙀 𝗞𝗮𝗧𝘆𝗔 - اوامــر المطـور المســاعد](t.me/D2_RK) .\n\n"
...
async def add_sudo_user(event):
    ...
    output += "**⎉╎يتم الان اعـادة تشغيـل بـوت كاتيا انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝗞𝗮𝗧𝘆𝗔 :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your D2_RK__.\n"
                )
            else:
    ...
    output = (
        result + "**⎉╎يتم الان اعـادة تشغيـل بـوت كاتيا انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
...
# Copyright (C) 2022 Source_Katya . All Rights Reserved
